...
export default {
  ...
  actions: {
    async getRegisterData({ commit, dispatch }, payload) {
      const APIkey = "AIzaSyB4fL3r4lb3psI5JB7uL9BEzsGRNGxxTCo";
      const authUrl =
    ...
    async addNewUser({ commit, state }, payload) {
      try {
        const { data } = await axios.post(
          `https://vue-js-project-7728b-default-rtdb.firebaseio.com/user.json?auth=${state.token}`,
          payload
        );
    ...
    async getUser({ commit }, payload) {
      try {
        const { data } = await axios.get(
          `https://vue-js-project-7728b-default-rtdb.firebaseio.com/user.json`)
          for (let key in data) {
